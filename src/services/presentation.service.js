...
const PRESENTATION_THEMES = {
  executive: {
    ...
    description: "Navy gelap dengan aksen emas â€” klasik untuk boardroom",
    colors: {
      background: "0B1F3A",
      surface: "132B4E",
      primary: "FFFFFF",
  ...
  slate: {
    id: "slate",
    label: "Slate Modern",
    description: "Slate gelap dengan aksen biru langit â€” bersih & futuristik",
    colors: {
      background: "0F172A",
      surface: "1E293B",
      primary: "FFFFFF",
  ...
  burgundy: {
    ...
    description: "Merah anggur dengan aksen krem â€” premium & elegan",
    colors: {
      background: "43101F",
      surface: "571B2C",
      primary: "FFFFFF",
...
const MAIIN_CONTACT = {
  ...
  hours: "Seninâ€“Jumat 08.00â€“22.00 WIB Â· Sabtuâ€“Minggu 06.00â€“22.00 WIB",
}

const SESSION_LABELS = {
  Morning: "Pagi (06â€“10)",
  Afternoon: "Siang (11â€“14)",
  Evening: "Sore (15â€“18)",
  Night: "Malam (19â€“23)",
}
...
const fmtIdr = (value) => `Rp ${Math.round(toNumber(value)).toLocaleString("id-ID")}`
...
const deltaLabel = (comparison) => {
  ...
  return comparison.changePct >= 0 ? `â–² +${pct}%` : `â–¼ ${pct}%`
}
...
const formatPeriodLabel = (data) => {
  ...
  try {
    return `${fmt(data.period.startDate)} â€“ ${fmt(data.period.endDate)}`
  } catch {
...
const formatComparisonPeriodLabel = (data) => {
  ...
  try {
    return `${fmt(data.comparisonPeriod.startDate)} â€“ ${fmt(data.comparisonPeriod.endDate)}`
  } catch {
    return data.comparisonPeriod.label || "periode sebelumnya"
  }
}

const sessionLabel = (sessionName) => SESSION_LABELS[sessionName] || sessionName || "â€”"

const courtLabel = (courtType) => {
...
const addSlideFooter = (pptx, slide, colors, pageNo, total) => {
  ...
  slide.addText("MAIIN GANDARIA â€¢ Management Report", {
    x: 0.7,
    y: 6.98,
    w: 8,
    h: 0.35,
...
const addExecutiveKpiSlide = (pptx, data, colors, totalSlides, pageNo) => {
  const slide = addSlideHeader(
    ...
    colors,
    "Ringkasan Eksekutif",
    `Periode ${formatPeriodLabel(data)} Â· dibandingkan ${formatComparisonPeriodLabel(data)}`
  )
  ...
  slide.addText(
    `Total jam terpakai: ${fmtCount(data.summary.courtHourCount)} dari ${fmtCount(data.summary.availableSessions)} slot tersedia Â· Rata-rata revenue per booking: ${fmtCompactIdr(data.summary.avgRevenuePerBooking)}`,
    {
      x: 0.75,
      y: 4.95,
      w: 11.8,
...
const addSessionPerformanceSlide = (pptx, data, colors, aiContent, totalSlides, pageNo) => {
  ...
  const renderSessionCard = (x, title, items, titleColor) => {
    ...
    slide.addText(
      items.map((item) => ({
        text: `${sessionLabel(item.sessionName)} Â· ${fmtPct(item.occupancyRate)} okupansi Â· ${fmtCompactIdr(item.revenue)}`,
...
const addInsightsSlide = (pptx, data, colors, aiContent, totalSlides, pageNo) => {
  ...
  insights.slice(0, 3).forEach((insight, index) => {
    ...
    if (insight.action) {
      slide.addText(`â†’ Aksi: ${insight.action}`, {
        x: 1.1,
        y: y + 1.18,
        w: 11.1,
        h: 0.4,
...
const addClosingSlide = (pptx, data, colors, totalSlides, pageNo) => {
  ...
  const contactLines = [
    `MAIIN Gandaria Â· ${MAIIN_CONTACT.address}`,
    MAIIN_CONTACT.phone,
    `Instagram ${MAIIN_CONTACT.instagram} Â· ${MAIIN_CONTACT.booking}`,
    MAIIN_CONTACT.hours,
  ]
